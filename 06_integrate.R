...
dt.sf2 = dt.sf %>% left_join(osm, by="id")
names(dt.sf2) <- substring(gsub('_','',names(dt.sf2)), 1, 10) # Problem with long column names and/or underscores
dt.sf2[is.na(dt.sf2)] = 0
st_write(dt.sf2, 'test2.shp', delete_dsn=TRUE, quiet=TRUE)

# Load NSPL
for (y in census.years) {
  nspl <- data.table::fread(get.path(paths$int,get.file(t="{file.nm}-{g.resolution}m-*-Grid.csv",'NSPL',y)))
  dt.sf3 = dt.sf %>% left_join(nspl, by="id")
  dt.sf3[is.na(dt.sf3)] = 0
