...
# Aviemore and related within Cairngorms National Park; various
...
for (k in ls(osm.classes)) {
  ...
  osm.amenity.extract = c('-f "ESRI Shapefile"', '-t_srs EPSG:27700', '-s_srs EPSG:4326', '-where "(amenity IS NOT NULL AND amenity NOT IN ({val}))"', file.step1, file.clip, '-overwrite', '--config ogr_interleaved_reading yes') 
  
  # Not used:
  #osm.tags.extract = c('-f "ESRI Shapefile"', '-t_srs EPSG:27700', '-s_srs EPSG:4326', '-where "{key} IN ({val})"', file.step1, file.clip, '-overwrite', '--config ogr_interleaved_reading yes')
  #/Library/Frameworks/GDAL.framework/Programs/ogr2ogr -f "ESRI Shapefile" -t_srs EPSG:27700 -s_srs EPSG:4326 -where "other_tags LIKE '%Forest%' OR other_tags LIKE '%Common%' OR other_tags LIKE '%Heath%'" /Users/jreades/Documents/github/popchange/no-sync/processed/Scotland-other_tags-step1.shp /Users/jreades/Documents/github/popchange/no-sync/OSM/Scotland-clip.shp -overwrite --config ogr_interleaved_reading yes
  ...
  if (k == 'not_null') {
    ...
    val = paste("'", paste(osm.classes[[k]], collapse="', '"), "'", sep="")
    cmd1 = osm.amenity.extract
    cmd2 = osm.union
  } else {
...
# system2(cmd3) # This seems works better than individual merge calls but still fails
sh.file = 'merge.sh'
print("Appending merge command to ./merge.sh")
cat(c("#!/bin/bash\n", paste(cmd3, collapse=" ")), file=sh.file, append=TRUE)
print(paste("Now running the shell script in",sh.file))
system2('sh' ,sh.file, wait=TRUE)
print(paste("Merged data will be in",file.merge))
