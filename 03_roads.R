...
if (r == 'Northern Ireland') {
  ...
  osgb.grid <- st_read( get.path(paths$grid.src,c('OSGB_Grids-master','Shapefile','OSGB_Grid_100km.shp')), quiet=TRUE, stringsAsFactors=FALSE) %>% st_set_crs(NA) %>% st_set_crs(crs.gb)
  ...
  rds       <- st_read(rds.fn, quiet=TRUE, stringsAsFactors=FALSE) %>% st_set_crs(NA) %>% st_set_crs(crs.gb)
  ...
  for (g in grid.tiles[2:length(grid.tiles)]) {
    ...
    if (file.exists(rds.fn)) {
      rds.shp <- st_read(rds.fn, quiet=TRUE, stringsAsFactors=FALSE) %>% st_set_crs(NA) %>% st_set_crs(crs.gb)
